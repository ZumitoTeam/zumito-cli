...
function createActionRowBuilderClass(filePath, name) {
    ...
    cls.addMethod({
        ...
        statements: `const { locale } = params;\n\nconst ${name}ActionRow = new ActionRowBuilder()\n    .addComponents(\n        new ButtonBuilder()\n            .setCustomId('${name.toLowerCase()}.primary')\n            .setLabel(this.translator.get('actionRows.${name}.primary', locale))\n            .setStyle(ButtonStyle.Secondary)\n    )\n    .addComponents(\n        new ButtonBuilder()\n            .setCustomId('${name.toLowerCase()}.secondary')\n            .setLabel(this.translator.get('actionRows.${name}.secondary', locale))\n            .setStyle(ButtonStyle.Secondary)\n    );\nreturn ${name}ActionRow;`,
    });

...
export const model = {
    ...
    action: async ({ moduleName, name }) => {
        ...
        createModelClass(path.join(modelsFolder, `${name}.ts`), name);
...
function createModelClass(filePath, name) {
    const project = new Project();
    const sourceFile = project.createSourceFile(filePath, "", { overwrite: true });

    sourceFile.addImportDeclaration({
        namedImports: ["Collection", "Field"],
    ...
    modelClass.addDecorator({
        name: "Collection",
        arguments: [`{ name: '${name.toLowerCase()}' }`],
    });

    // Add a sample id field
    const idProp = modelClass.addProperty({
        name: "id",
        type: "string",
    });
    idProp.addDecorator({
        name: "Field",
        arguments: ["{ type: 'string', primary: true }"],
    });

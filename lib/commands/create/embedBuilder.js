...
function createEmbedBuilderClass(filePath, name) {
    ...
    cls.addMethod({
        ...
        statements: `const { locale } = params;\n\nconst ${name}Embed = new EmbedBuilder()\n    .setTitle(this.translator.get('embeds.${name}.title', locale))\n    .setDescription(this.translator.get('embeds.${name}.description', locale))\n    .setColor(config.colors.primary)\n    .setThumbnail(config.client.user.avatarURL())\n    .setFooter({ text: config.client.user.tag, iconURL: config.client.user.avatarURL() })\n    .setTimestamp();\n\nreturn ${name}Embed;`,
    });

...
import { findConfigObject, getModulesArray, resolveElementName } from '../../utils/configEditor.js';
...
const registerModules = async (configPath, modules) => {
    ...
    let changed = false;
    if (!modulesProp) {
        modulesProp = configObject.addPropertyAssignment({ name: 'modules', initializer: '[]' });
        changed = true;
    }
    ...
    elements.forEach((element, index) => {
        ...
        if (!existingTexts.has(name)) {
            existingTexts.set(name, element.getText());
        }
        targetInfos.push({ pkg: name, element, index });
    });
    ...
    for (const pkg of orderedPackages) {
        if (!existingTexts.has(pkg)) changed = true;
